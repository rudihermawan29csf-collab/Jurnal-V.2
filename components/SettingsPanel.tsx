...
const SettingsPanel: React.FC<SettingsPanelProps> = ({ 
  ...
  const handleBackupData = () => {
    const allData = {
      ...
      unavailableConstraints,
      students,
      // Note: This assumes parent component passes these down or we access localStorage directly
      // Accessing localStorage for complete backup
      scheduleMap: JSON.parse(localStorage.getItem('scheduleMap') || '{}'),
      teachingMaterials: JSON.parse(localStorage.getItem('teachingMaterials') || '[]'),
      teachingJournals: JSON.parse(localStorage.getItem('teachingJournals') || '[]'),
      studentGrades: JSON.parse(localStorage.getItem('studentGrades') || '[]'),
      homeroomRecords: JSON.parse(localStorage.getItem('homeroomRecords') || '[]'),
  ...
  const handleRestoreData = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if(data.teacherData) localStorage.setItem('teacherData', JSON.stringify(data.teacherData));
        if(data.scheduleMap) localStorage.setItem('scheduleMap', JSON.stringify(data.scheduleMap));
        if(data.teacherLeaves) localStorage.setItem('teacherLeaves', JSON.stringify(data.teacherLeaves));
        if(data.calendarEvents) localStorage.setItem('calendarEvents', JSON.stringify(data.calendarEvents));
        if(data.unavailableConstraints) localStorage.setItem('unavailableConstraints', JSON.stringify(data.unavailableConstraints));
        if(data.students) localStorage.setItem('students', JSON.stringify(data.students));
        if(data.teachingMaterials) localStorage.setItem('teachingMaterials', JSON.stringify(data.teachingMaterials));
        if(data.teachingJournals) localStorage.setItem('teachingJournals', JSON.stringify(data.teachingJournals));
        if(data.studentGrades) localStorage.setItem('studentGrades', JSON.stringify(data.studentGrades));
        if(data.homeroomRecords) localStorage.setItem('homeroomRecords', JSON.stringify(data.homeroomRecords));
          ...
          {activeTab === 'GENERAL' && (
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="mt-8 pt-6 border-t-2 border-gray-100">
                 <h3 className="text-lg font-bold text-gray-800 mb-2">Backup & Restore Data</h3>
                 <p className="text-sm text-gray-500 mb-4">Simpan cadangan data ke file lokal (komputer) atau pulihkan data dari file. Berguna jika koneksi Spreadsheet bermasalah.</p>
                 
                 <div className="flex flex-col md:flex-row gap-4">
                    <button 
                       onClick={handleBackupData}
                       className="flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-lg font-bold hover:bg-emerald-700 shadow-sm transition-colors"
                    >
                       <FileJson size={20} /> Backup Data (JSON)
                    </button>
                    ...
                    <div className="relative">
                       ...
                       />
                       <button className="flex items-center justify-center gap-2 px-6 py-3 bg-orange-600 text-white rounded-lg font-bold hover:bg-orange-700 shadow-sm transition-colors w-full">
                          <UploadCloud size={20} /> Restore Data (JSON)
                       </button>
                    </div>
                 </div>
              </div>
